...
# %%
class SuperHyperbolicAnisotropicEnumerateFunction(ot.HyperbolicAnisotropicEnumerateFunction):
    ...
    def getMaximumDegreeStrataIndexNew(self, maximumDegree: int) -> int:  # <- New
        ...
        while (True):
            ...
            multiindices_total_degree_in_layer = [sum(enumerate(idx + number_of_coefficients)) for idx in range(strata_cardinal)]
            maximum_degree_in_layer = max(multiindices_total_degree_in_layer)
            print(f"In strata index {strata_index}, "
