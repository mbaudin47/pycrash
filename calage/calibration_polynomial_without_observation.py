# -*- coding: utf-8 -*-
"""
Adapted from t_GaussianLinearCalibration_noobs.py
"""
...
# This parametricModel is linear in (a, b, c) and identifiable.
...
parametricModel = ot.ParametricFunction(g, calibratedIndices, candidate)

# See the parametric model in action
oneSingleInputObservation = []
...
if False:
    """
    Cas 1 : sans entrée observée du tout
    TypeError: Wrong number or type of arguments for overloaded 
    function 'new_GaussianLinearCalibration'.
    """
    algo = ot.GaussianLinearCalibration(
        g, outputObservations, candidate, priorCovariance, errorCovariance,
        method
    )

    """
    Cas 2 : avec entrée observée, de dimension nulle
    TypeError: InvalidArgumentException : Error: expected a model of parameter 
    dimension=3, got parameter dimension=0
    """
    algo = ot.GaussianLinearCalibration(
        g, inputObservations, outputObservations, candidate, priorCovariance, errorCovariance,
        method
    )
