...
Gaussien linéaire lorsque le nombre d'observations augmente.
...
log_size = np.logspace(1.1, 3.5, number_of_experiments)
...
for i in range(dimension):
    ...
    graph.add(curve)
    # 3. The sample of candidate parameters
    theta_true = ot.Sample([[candidate[i]] for j in range(number_of_experiments)])
    curve = ot.Curve(size_sample, theta_true)
    graph.add(curve)
...
figure.savefig("linear_calibration_Gaussian_theta.pdf", bbox_inches="tight")
...
figure.savefig("linear_calibration_Gaussian_eabs.pdf", bbox_inches="tight")
