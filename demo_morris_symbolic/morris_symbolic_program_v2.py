...
def build_morris_function(b0_random=0.0, b1_random=ot.Point(10), b2_random=ot.Point(175)):
   ...
   >>> morrisFunction = build_morris_function(b0_random, b1_random, b2_random)
   ...
   b1_expr = ",".join(fmt(v) for v in b1)
   b2_expr = ",".join(fmt(v) for v in b2_random)
...
g = build_morris_function(b0, b1, b2)
