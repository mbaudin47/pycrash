...
model_evaluations = (
    parameter_samples[0] * numpy.e ** -numpy.outer(coordinates, parameter_samples[1])
).T  # this is faster
...
true_variance = (
    numpy.hstack([2.29, 11.45 * (numpy.e ** (-0.2 * _t) - numpy.e ** (-0.4 * _t)) / _t])
    - true_mean ** 2
)
...
def error_in_variance(predicted_variance, true_variance=true_variance):
    """
...
eps_mean = [error_in_mean(numpy.mean(model_evaluations[:idx], 0)) for idx in indices]
eps_variance = [
    error_in_variance(numpy.var(model_evaluations[:idx], 0)) for idx in indices
]
