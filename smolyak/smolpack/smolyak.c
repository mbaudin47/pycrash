...
double
int_smolyak (int dim, int qq, double (*ff) (int, double xx[]),
	     int print_stats)
/******************************************************************************/
/*
  Purpose:
  ...
  if (r == s)
      ...
      if (sw[l] < fn)
	{
	  if (sw[l] == 0)
		...
		{
		  total =
		    dnu[sw[l]][invlook[sw[l]]
			       [lookind[indices[r]][argind[r]]]];
		}
	    }
	}
    }
  ...
  if (k == 0)
      ...
      for (i = 1; i <= d; i++)
	{
	  if (indices[i] == 0)
	    ...
	    {
	      wprod = wprod *
		dnu[wind[i]][invlook[wind[i]]
			     [lookind[indices[i]][argind[i]]]];
	    }
	}
      wsum = wsum + wprod;
    }
